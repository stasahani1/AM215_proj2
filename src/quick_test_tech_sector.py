"""
Quick test with TECH SECTOR ONLY stocks.

Hypothesis: Contagion should be stronger within a sector than across sectors.
If SIR model performs better with same-sector stocks, it validates sector-specific contagion.
"""
...
YEARS_BACK = 10
CHUNK_DAYS = 30  # 6 weeks (found to work well)
N_CHUNKS = 30
TRAIN_RATIO = 0.6
N_SIMULATIONS = 200

print("="*80)
print("TECH SECTOR ONLY TEST")
print("="*80)
print(f"\nHypothesis: SIR contagion should be STRONGER within tech sector")
print(f"\nConfiguration:")
print(f"  Stocks: {len(TECH_STOCKS)} tech stocks")
...
# Training: Mix of all VIX levels (to learn states properly)
n_train = int(N_CHUNKS * TRAIN_RATIO)
n_test = N_CHUNKS - n_train

# Get diverse training set (every nth chunk to get mix of VIX levels)
...
if sir_model.is_trained:
    ...
    print(f"\nExpectation: β should be HIGHER for tech-only stocks if contagion is sector-specific")
...
print("\n" + "="*80)
print("Backtest Summary - TECH SECTOR ONLY")
print("="*80)
...
if sir_mse < best_baseline_mse:
    ...
    print(f"   This supports sector-specific contagion!")
else:
    print(f"\n❌ SIR underperforms by {-improvement:.1f}%")
    print(f"   Even within tech sector, contagion effects are weak")
